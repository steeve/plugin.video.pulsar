...
def request(url, params={}, headers={}, data=None, method=None, with_cookies=False):
    if params:
        import urllib
        url = "".join([url, "?", urlencode(params)])

    if with_cookies and COOKIE_OPENER is None:
...
HEAD = lambda *args, **kwargs: request(*args, method="POST", **kwargs)
...
def get_setting(self, key, converter=str, choices=None):
...
def register(search, search_movie, search_episode):
    ...
    try:
        ...
        # notify("This addon can only be run from Pulsar", time=5000)
        return
